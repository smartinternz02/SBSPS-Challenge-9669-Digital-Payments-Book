...
if(mysqli_num_rows($result)>0)
{
  ...
  echo('<tr>         
                    <td><button type="button" class="btn btn-white btn-sm btn-round waves-effect waves-light m-1" onclick="comp('.$row['ID'].')">Complete</button></td>
                    <td>'.$row['ID'].'</td>
                    <td>'.$row['ORDER_ID'].'</td>
                    <td>'.$row['C_ID'].'</td>');
  $s=$row['C_ID'];
